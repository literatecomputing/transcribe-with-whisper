...
def convert_html_string_to_docx(html: str, out_path: Path) -> None:
    ...
    for inner in segments:
        speaker = ""
        timestamp = ""
        m = re.search(r'<span[^>]*\bclass=[\'\"]?[^>]*\bspeaker-name\b[^>]*[\'\"]?[^>]*>(.*?)</span>', inner, flags=re.S | re.I)
        if m:
            speaker = _strip_tags(m.group(1))

        m = re.search(r'<span[^>]*\bclass=[\'\"]?[^>]*\btimestamp\b[^>]*[\'\"]?[^>]*>(.*?)</span>', inner, flags=re.S | re.I)
        if m:
            timestamp = _strip_tags(m.group(1))

        m = re.search(r'<span[^>]*\bclass=[\'\"]?[^>]*\btranscript-text\b[^>]*[\'\"]?[^>]*>(.*?)</span>', inner, flags=re.S | re.I)
        if m:
